...
public class PaymentController {
    ...
    @GetMapping(value = "/generatePaymentReportHtml",produces = MediaType.TEXT_HTML_VALUE)
    public @ResponseBody String generatePaymentReportHtml(@RequestParam("from")@DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate from,
                                                  @RequestParam("to")@DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate to ){
        return paymentReportHtmlGenerator.generateHtmlTemplate(
                hisaabService.generatePaymentReport(from,to)
        );
