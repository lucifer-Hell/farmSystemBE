...
public class PaymentReportHtmlGenerator {
    public String generateHtmlTemplate(PaymentReportDto paymentReportDto){
        ...
        String htmlString = "<!DOCTYPE html><head> <title>Payment Report</title> <style> table { border-collapse: collapse; width: 100%; } th, td { padding: 8px; text-align: center; border: 1px solid #000; } th { background-color: #f2f2f2; } tbody tr:nth-child(even) { background-color: #f9f9f9; } tbody tr:hover { background-color: #eaf2ff; } </style></head><body> <table border=\"3\">" +
    ...
    private String generateRecordRows(PaymentReportDto paymentReportDto) {
        StringBuilder recordHtml=new StringBuilder("<tr>");
        paymentReportDto.getPaymentReportRows().forEach(paymentReportRowDto -> {
            ...
            paymentReportDto.getFrom().datesUntil(paymentReportDto.getTo().plusDays(1)).forEach(date -> {
                if(!paymentReportRowDto.getDateShiftMap().get(date).isEmpty()){
    ...
    private String generateTableHeading(PaymentReportDto paymentReportDto) {
        long attendenceColSpan= Duration.between(paymentReportDto.getFrom(),paymentReportDto.getTo().plusDays(1)).toDays();
